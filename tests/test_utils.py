...
class TestUtilsCompareFiles(object):
    def test_doesnt_exists_file1(self):
        assert IcecrustUtils.compare_files(TEST_DIR + 'foobar.txt', TEST_DIR + 'file1.txt', verbose=True) is False
...
class TestUtilsVerifyChecksum(object):
    ...
    def test_doesnt_exists_no_checksums(self):
        assert IcecrustUtils.verify_checksum(TEST_DIR + 'file1.txt', DEFAULT_HASH_ALGORITHM) is False
